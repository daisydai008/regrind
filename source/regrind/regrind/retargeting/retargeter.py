...
class HandInteractionMeshOneStageRetargeter:
    ...
    def _calc_manipulator_jacobians(
        ...
        """Compute the Jacobian w.r.t. to the link pose in either object frame or world frame
        through autodiff."""
        J_XC_dict = {}
        p_XC_dict = {}
        ...
        for name, link_name in links.items():
            ...
            P_WC = self.plant_ad.GetBodyByName(link_name).body_frame().CalcPoseInWorld(self.plant_context_ad)

            if obj_frame:
                p_XC = (P_WO.inverse().multiply(P_WC)).translation()
            else:
                p_XC = P_WC.translation()

            J_XC_dict[name] = ExtractGradient(p_XC)[:, self.q_a_indices]
    ...
    def _get_robot_link_positions(self, q, link_names):
        """Get robot link positions for given configuration."""
        self.plant.SetPositions(self.plant_context, q)

        robot_link_positions = []
        for link_name in link_names:
            robot_link_positions.append(
